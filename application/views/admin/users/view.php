              ...
              <div class="card-header d-flex p-0">
                ...
                <ul class="nav nav-pills ml-auto p-2">
					...
					<?php if (hasPermissions('users_edit')): ?>
						<li class="nav-item"><a class="nav-link" href="<?php echo url('admin/users/edit/'.$User->id) ?>"><?php echo lang('edit_user') ?></a></li>
					<?php endif ?>
      		...
      		<div class="col-sm-10" style="padding-left: 50px;">
				  ...
				  <table id="dataTable1" class="table table-bordered table-striped">
					...
					<?php foreach ($User->activity as $row): ?>
						...
						<td><?php echo !empty($row->ip_address)?'<a href="'.url('admin/activity_logs/index?ip='.urlencode($row->ip_address)).'">'.$row->ip_address.'</a>':'N.A' ?></td>
						<td>
							<a href="<?php echo url('admin/activity_logs/view/'.$row->id) ?>"><?php echo $row->title ?></a>
						</td>
						<td><?php echo date('d M, Y', strtotime($row->created_at)) ?></td>
						<td>
							<a href="<?php echo url('admin/activity_logs/view/'.$row->id) ?>" class="btn btn-sm btn-default" title="View Activity" data-toggle="tooltip"><i class="fa fa-eye"></i></a>
						</td>
						</tr>
					<?php endforeach ?>
