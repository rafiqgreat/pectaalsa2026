  ...
  <div class="row">
    ...
    <div class="col-sm-9">
      ...
      <div class="card">
        ...
        <?php echo form_open_multipart('admin/settings/generalUpdate', [ 'class' => 'form-validate', 'autocomplete' => 'off', 'method' => 'post' ]); ?>
        <div class="card-body">
          ...
          <br>
          <h4>Block Users/Applicant Access</h4>
          <hr>
          <div class="form-group">
            <label class="d-block mb-2" for="user_access_blocked">User Side Access Status</label>
            <input
              type="checkbox"
              id="user_access_blocked"
              name="user_access_blocked"
              value="1"
              class="js-switch"
              <?php echo user_access_blocked() ? 'checked' : '' ?>
            />
            <span class="ml-2 badge <?php echo user_access_blocked() ? 'badge-danger' : 'badge-success' ?>">
              <?php echo user_access_blocked() ? 'Blocked' : 'Unblocked' ?>
            </span>
            <small class="form-text text-muted">
              When enabled, applicant/user login and applicant-side pages will be blocked.
            </small>
            <div class="mt-2">
              <button type="button" id="toggleUserAccessBtn" class="btn btn-sm <?php echo user_access_blocked() ? 'btn-success' : 'btn-danger' ?>">
                <?php echo user_access_blocked() ? 'Unblock Access' : 'Block Access' ?>
              </button>
              <small class="form-text text-muted">Click the button, then submit to apply the change.</small>
            </div>
          </div>

          <div class="form-group">
            <label for="user_access_block_message">Blocked Access Message</label>
            <textarea
              class="form-control"
              name="user_access_block_message"
              id="user_access_block_message"
              rows="3"
              placeholder="Applicants/ Users Access is Blocked Currently Please Wait"
            ><?php echo htmlspecialchars(user_access_block_message(), ENT_QUOTES, 'UTF-8'); ?></textarea>
            <small class="form-text text-muted">
              This message will be shown in red on the applicant login page.
            </small>
          </div>
...
elems.forEach(function(html) {
  ...
  function updateUserAccessToggleButton() {
    var blocked = $('#user_access_blocked').is(':checked');
    $('#toggleUserAccessBtn')
      .toggleClass('btn-danger', !blocked)
      .toggleClass('btn-success', blocked)
      .text(blocked ? 'Unblock Access' : 'Block Access');
  }

  $(document).on('change', '#user_access_blocked', updateUserAccessToggleButton);
  $(document).on('click', '#toggleUserAccessBtn', function () {
    document.getElementById('user_access_blocked').click();
  });
  updateUserAccessToggleButton();
</script>
